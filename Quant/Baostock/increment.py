...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('increment_import.log')
    ]
)
logger = logging.getLogger(__name__)

# 全局配置
HISTORICAL_START_DATE = "2020-01-01"  # 历史数据起始日期
...
def get_existing_date_range():
    """获取ClickHouse中已有的日期范围（同update.py）"""
    try:
        ch_client = Client(
            host='localhost',
            port=9000,
...
def bulk_insert_to_clickhouse(client, df):
    ...
    try:
        ...
        logger.info(f"成功写入 {total_inserted} 条增量历史数据")
        return total_inserted
    except Exception as e:
        logger.error(f"批量写入失败: {str(e)}")
        raise

def import_historical_data(start_date, end_date, stock_codes):
    """导入历史数据主流程"""
    logger.info(f"开始导入历史数据: {start_date} 到 {end_date}")
...
def main():
    """主函数：增量导入历史数据"""
    logger.info("="*50)
    logger.info("开始增量导入历史数据...")
    logger.info("="*50)
    
    # 获取现有数据范围
    min_date, _ = get_existing_date_range()
    ...
    # 计算需要导入的日期范围
    target_start = datetime.strptime(HISTORICAL_START_DATE, "%Y-%m-%d").date()
    target_end = min_date - timedelta(days=1)
    
    if target_start >= min_date:
        logger.info(f"数据已包含从 {HISTORICAL_START_DATE} 开始的历史数据，无需增量导入")
        return
    
    logger.info(f"需要导入的历史数据范围: {target_start} 到 {target_end}")
    ...
    total_rows, failed_codes = import_historical_data(
        str(target_start), 
        str(target_end), 
        all_codes
    )
