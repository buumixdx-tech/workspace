...
from models import TimeRule, MaterialInventory, SKU, SystemEvent, Point
...
def time_rule_completion_listener(session: Session, event: SystemEvent):
    ...
    # 直接在写入 trigger_time 时计算并写入 flag_time
    ...
    for rule in rules:
        ...
        if rule.offset is not None:
            rule.flag_time = evaluator._calculate_flag_time(now, rule.offset, rule.unit)
    ...
    emit_event(session, SystemEventType.RULES_TRIGGERED_BY_LOGISTICS, SystemAggregateType.TIME_RULE, vc_id, {
        ...
        "transaction_date": payload.get("transaction_date") if payload else None
    })
