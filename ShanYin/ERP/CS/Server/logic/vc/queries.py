...
from models import get_session, VirtualContract, Business, ChannelCustomer, TimeRule, VirtualContractStatusLog, CashFlow, PartnerRelation, ExternalPartner, SKU
...
def get_vc_detail(vc_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if elements:
            el_list = elements.get("items") or elements.get("elements") or []
            sku_ids = set()
            for el in el_list:
                if el.get("sku_id"):
                    sku_ids.add(el["sku_id"])
            if sku_ids:
                sku_rows = session.query(SKU).filter(SKU.id.in_(sku_ids)).all()
                sku_map = {s.id: s.name for s in sku_rows}
                for el in el_list:
                    sid = el.get("sku_id")
                    if sid and sid in sku_map:
                        el["sku_name"] = sku_map[sid]
        ...
        # 物料供应类型 VC，查询关联的合作方关系（采购执行类型）
        if vc.type == VCType.MATERIAL_SUPPLY and vc.business_id:
...
def get_virtual_contracts_for_return(
    ...
    try:
        ...
        # 获取所有涉及的供应链ID和客户ID
        sc_ids = set(c.supply_chain_id for c in contracts if c.supply_chain_id)
        business_ids = set(c.business_id for c in contracts if c.business_id)

        # 批量查询供应链及其供应商名称
        from models import SupplyChain, Supplier
        sc_supplier_map = {}  # sc_id -> supplier_name
        if sc_ids:
            chains = session.query(SupplyChain).filter(SupplyChain.id.in_(sc_ids)).all()
            for sc in chains:
                if sc.supplier_id:
                    sup = session.query(Supplier).get(sc.supplier_id)
                    sc_supplier_map[sc.id] = sup.name if sup else None

        # 批量查询客户名称
        from models import ChannelCustomer, Business
        customer_map = {}
        if business_ids:
            businesses = session.query(Business).filter(Business.id.in_(business_ids)).all()
            customer_ids = set(b.customer_id for b in businesses if b.customer_id)
            if customer_ids:
                customers = session.query(ChannelCustomer).filter(ChannelCustomer.id.in_(customer_ids)).all()
        ...
        for c in contracts:
            # 计算交易对手
            vc_type = c.type
            if vc_type in ('设备采购', '库存采购', '物料采购'):
                counterparty = sc_supplier_map.get(c.supply_chain_id) or '-'
            elif vc_type in ('物料供应', '库存拨付'):
                biz = session.query(Business).get(c.business_id) if c.business_id else None
                customer_name = customer_map.get(biz.customer_id) if biz and biz.customer_id else None
                counterparty = customer_name or '-'
            else:
                counterparty = '-'
