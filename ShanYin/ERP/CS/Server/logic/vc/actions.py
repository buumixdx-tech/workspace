...
def _record_vc_created_log(session: Session, vc: VirtualContract, created_date=None):
    """VC创建时记录初始状态日志（status=执行），同时更新status_timestamp"""
    vc.update_status(vc.status, is_initial=True, transaction_date=created_date)
    ...
    try:
        with audit_context("create_procurement_vc_action"):
            ...
            new_vc = VirtualContract(
                ...
                transaction_date=payload.created_date
            )
            session.add(new_vc)
            session.flush()
            ...
            biz_details = dict(biz.details) if biz.details else {}
            biz_pricing = dict(biz_details.get("pricing", {}))
            biz_pricing.update(orig_pricing)
            if biz_pricing != biz_details.get("pricing", {}):
                biz_details["pricing"] = biz_pricing
                biz.details = biz_details
                _attributes.flag_modified(biz, "details")

            _record_vc_created_log(session, new_vc, payload.created_date)
            ...
            evt.payload = {"type": new_vc.type, "business_id": new_vc.business_id, "total_amount": payload.total_amt, "transaction_date": str(payload.created_date) if payload.created_date else None}
...
def create_material_supply_vc_action(session: Session, payload: CreateMaterialSupplyVCSchema, draft_rules: list = None) -> ActionResult:
    ...
    try:
        with audit_context("create_material_supply_vc_action"):
            ...
            batch_check_items = []   # (sku_id, batch_no, point_id, qty) — 精确批次维度校验
            clean_elems = []
            for e in payload.elements:
                # 校验收货点
                if e.receiving_point_id not in allowed_receiving:
                ...
                clean_elems.append(_elem_to_dict(corrected))
                # 精确批次维度：同 (sku_id + batch_no + point_id) 的 element 合计超量检查
                batch_check_items.append((e.sku_id, e.batch_no, e.shipping_point_id, e.qty))

            # 应用附加业务政策（价格覆盖 + 标记 addon_business_ids）
            clean_elems = _apply_addons_to_elements(session, payload.business_id, clean_elems)

            # 精确批次维度库存校验：同 (sku_id + batch_no + point_id) 合计不能超过该批次库存
            from logic.services import validate_batch_inventory_availability
            is_ok, over_stock = validate_batch_inventory_availability(session, batch_check_items)
            if not is_ok:
                return ActionResult(success=False, error=f"批次库存不足: {' | '.join(over_stock)}")
    ...
    try:
        with audit_context("create_return_vc_action"):
            ...
            evt.payload = {
                ...
                "total_refund": payload.total_refund,
                "transaction_date": str(payload.created_date) if payload.created_date else None,
            }
...
def create_mat_procurement_vc_action(session: Session, payload: CreateMatProcurementVCSchema, draft_rules: list = None) -> ActionResult:
    ...
    try:
        with audit_context("create_mat_procurement_vc_action"):
            ...
            evt.aggregate_id = new_vc.id
            evt.payload = {"type": new_vc.type, "total_amount": payload.total_amt, "transaction_date": str(payload.created_date) if payload.created_date else None}
...
def create_inventory_allocation_action(session: Session, payload: AllocateInventorySchema) -> ActionResult:
    ...
    try:
        with audit_context("create_inventory_allocation_action"):
            ...
            evt.aggregate_id = new_vc.id
            evt.payload = {"type": new_vc.type, "equipment_count": sum(e.qty for e in payload.elements), "transaction_date": str(payload.created_date) if payload.created_date else None}
