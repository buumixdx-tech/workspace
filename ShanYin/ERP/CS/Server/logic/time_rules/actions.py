...
from logic.constants import SystemEventType, SystemAggregateType, EventType
from datetime import datetime

def save_rule_action(session: Session, payload: TimeRuleSchema) -> ActionResult:
    """保存或更新时间规则 Action"""
    try:
        # 绝对日期模式必须指定 flag_time
        if (payload.trigger_event == EventType.Special.ABSOLUTE_DATE
                and not payload.flag_time):
            return ActionResult(success=False, error="绝对日期模式必须指定 flag_time")
