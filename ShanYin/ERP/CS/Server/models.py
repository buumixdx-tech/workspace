...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Date, ForeignKey, Text, JSON, Boolean, Index, UniqueConstraint
...
try:
    import orjson
    def _orjson_serializer(value):
        return orjson.dumps(value).decode('utf-8')
    def _orjson_deserializer(value):
        return orjson.loads(value)
except ImportError:
    import json
    _orjson_serializer = json.dumps
    _orjson_deserializer = json.loads


class _FrozenDict(dict):
    """不可变 dict 子类实现，hashable（基于 id），用于解决 SQLAlchemy 查询去重时 dict 不可哈希的问题。"""
    __slots__ = ()

    def __setitem__(self, *args, **kwargs):
        raise TypeError("'_FrozenDict' object does not support item assignment")

    def __delitem__(self, *args, **kwargs):
        raise TypeError("'_FrozenDict' object does not support item deletion")
...
class OrJsonType(TypeDecorator):
    """使用 orjson 序列化的 JSON 类型，返回不可变 dict，解决 dict 不可哈希导致的 SQLAlchemy 去重错误。"""
    impl = Text
    cache_ok = True
...
class User(Base):
    ...
    permissions = Column(OrJsonType, default=dict)  # 权限配置，存储可访问的端点列表
...
class Supplier(Base):
    ...
    info = Column(OrJsonType) # 额外资质或明细
...
class SKU(Base):
    ...
    params = Column(OrJsonType) # 参数 JSON
...
class Contract(Base):
    ...
    parties = Column(OrJsonType) # 签约方信息
    content = Column(OrJsonType) # 合同详情
...
class VirtualContract(Base):
    ...
    summary = Column(Text)
    elements = Column(OrJsonType)  # 结构因 VC 类型而异，详见 logic/vc/schemas.py 各 CreateVCSchema 文档
    return_direction = Column(String(50), nullable=True) # 退货方向：CUSTOMER_TO_US / US_TO_SUPPLIER
    deposit_info = Column(OrJsonType)  # 押金信息，结构因 VC 类型而异（设备采购有值，库存/物料采购无此字段），详见 logic/vc/schemas.py
    ...
    transaction_date = Column(Date)  # 业务发生日期，与 SystemEvent VC_CREATED payload.transaction_date 保持一致
    ...
    def _add_status_log(self, category, old_val, new_val, transaction_date=None):
    ...
    def update_status(self, new_val, is_initial=False, transaction_date=None):
        old_val = self.status
        if old_val == new_val and not is_initial: return
        self.status = new_val
        self._add_status_log("status", None if is_initial else old_val, new_val, transaction_date)

    def update_subject_status(self, new_val, is_initial=False, transaction_date=None):
        old_val = self.subject_status
        if old_val == new_val and not is_initial: return
        self.subject_status = new_val
        self._add_status_log("subject", None if is_initial else old_val, new_val, transaction_date)

    def update_cash_status(self, new_val, is_initial=False, transaction_date=None):
        old_val = self.cash_status
        if old_val == new_val and not is_initial: return
        self.cash_status = new_val
        self._add_status_log("cash", None if is_initial else old_val, new_val, transaction_date)
...
class VirtualContractStatusLog(Base):
    ...
    timestamp = Column(DateTime, default=datetime.now)
    transaction_date = Column(Date, nullable=True)  # 业务发生日期
...
class VirtualContractHistory(Base):
    ...
    id = Column(Integer, primary_key=True)
    vc_id = Column(Integer, ForeignKey('virtual_contracts.id'))
    original_data = Column(OrJsonType)
...
class BankAccount(Base):
    ...
    account_info = Column(OrJsonType)
...
class Business(Base):
    ...
    timestamp = Column(DateTime, default=datetime.now)
    details = Column(OrJsonType) # 记录业务演进历史
...
class SupplyChain(Base):
    ...
    payment_terms = Column(OrJsonType)  # 兼容旧版
    # elements = Column(OrJsonType)      # 新版：存储 pricing_config, payment_terms 等
...
class ExpressOrder(Base):
    ...
    items = Column(OrJsonType) # SKU、数量
    address_info = Column(OrJsonType)
...
class CashFlow(Base):
    ...
    payment_info = Column(OrJsonType) # 外部参考 / 交易代码
...
class SystemEvent(Base):
    ...
    payload = Column(OrJsonType)              # 关键快照数据
    created_at = Column(DateTime, default=datetime.now)
